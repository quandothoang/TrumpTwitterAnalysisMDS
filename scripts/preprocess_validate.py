...
Usage: preprocess_validate.py --raw_data=<raw_data> --write_to=<write_to>

Options:
--raw_data=<raw_data>   Path to raw CSV file
--write_to=<write_to>   Path to save processed CSV file
"""
...
@click.command()
@click.option('--raw_data', type=str, required=True, help='Path to raw CSV file')
@click.option('--write_to', type=str, required=True, help='Path to save processed CSV file')
def main(raw_data: str, write_to: str):
    ...
    print("DATA SUMMARY")
    print(f"Date range: {tweets.index.min()} to {tweets.index.max()}")
    print(f"Total tweets: {len(tweets)}")
    print(f"\nTime of day distribution:")
    for tod, count in tweets["time_of_day"].value_counts().items():
        print(f"  {tod}: {count:,} ({count / len(tweets) * 100:.1f}%)")
    print(f"\nSeason distribution:")
    for season, count in tweets["season"].value_counts().items():
        print(f"  {season}: {count:,} ({count / len(tweets) * 100:.1f}%)")
