...
Markdown(tweet_time.to_markdown())
```
...
Markdown(tweet_season.to_markdown())
```

```{python}
#| label: fig-time-of-day
#| fig-cap: "Trump's tweet frequency by time of day."
#| fig-width: 8
#| fig-height: 4

time_bars = (
    alt.Chart(tweet_time)
    .mark_bar(color="#f00808")
    .encode(
        y="Time:N",
        x="Count:Q",
    )
...
final_time_of_day_chart = (
    time_bars + range_text + count_text
).properties(padding={"right": 50, "top": 15, "bottom": 15, "left": 10})
...
```{python}
#| label: fig-season-frequency
#| fig-cap: "Trump's tweet frequency by season."
#| fig-width: 8
#| fig-height: 4

season_bars = (
    alt.Chart(tweet_season)
    .mark_bar(color="#f00808")
    .encode(
        x="Count:Q",
...
final_season_chart = (
    season_bars + season_range_text + season_count_text
).properties(padding={"right": 50, "top": 15, "bottom": 15, "left": 10})
...
```{python}
#| label: fig-time-and-season
#| fig-cap: "Trump's tweet frequency by time of day and season."
#| fig-width: 10
#| fig-height: 4

tweet_charts = (tweet_seasons | tweet_times).properties(
    padding={"right": 50, "top": 25, "bottom": 25, "left": 25}
)
tweet_charts
```
...
Markdown(sentiment_counts.to_markdown())
```
...
#| fig-cap: "Number of tweets per sentiment (positive, neutral, negative)."
#| fig-width: 6
#| fig-height: 4
...
Markdown(top_pos_df.to_markdown())
...
#| fig-cap: "Word cloud of positive tweets"
#| label: fig-pos-wordcloud
...
#| fig-cap: "Word cloud of negative tweets"
#| label: fig-neg-wordcloud
