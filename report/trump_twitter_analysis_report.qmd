...
![Number of tweets per sentiment (positive, neutral, negative).](../results/figures/sentiment_counts.png){#fig-sentiment-dist height=50%}

The actual context and literal meaning of the tweets was not analyzed, nor the accuracy of the classification at this point, so there is certainly going to be a margin of error and some 'false positives' etc as the positivity is quite often a self directed compliment of sorts. For the second method of sentiment analysis we chose to combine the neutral category with the positive category as neutral is positive in the way that it is not explicitly provocative or aggressive in manner. This will of course inflate the positive category for the next sentiment analysis but could possibly add positive words to the word cloud.
...
The second classifier method used to analyze the tweets was utilizing the CountVectorizer() to classify sets as positive words, negative words and some stop words unique to this dataset. The overall positive VS negative tweet counts are skewed in the opposite direction as the 'neutral' category here is somewhat added to the postively classified category. This choice was made because our 
primary goal was to identify words strongly associated with negative sentiment, and neutral tweets share more linguistic features with positive tweets than negative ones.
```{python}
...
- VADER may not capture sarcasm, irony, or context-dependent sentiment.
- Neutral tweets were merged with positive, which may inflate the "positive" category.
- The logistic regression model was trained on weak labels, not manually verified sentiment.
- Word clouds show individual word frequencies but not phrases or context.

## Conclusion

This analysis demonstrates that Trump's Twitter activity varied significantly by time and season, with peak activity in evenings and summer months. While most tweets were classified as positive, the sentiment analysis reveals distinct vocabulary patterns between positive and negative tweets that align with his political messaging style.
